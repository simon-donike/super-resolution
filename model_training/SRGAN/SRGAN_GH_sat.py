...
def main():
    ...
    dataset_train = dataset(folder_path,dataset_file,test_train_val="train",transform=transform,sen2_amount=1, location=location)
    train_loader = DataLoader(dataset_train,batch_size=batch_size, shuffle=True, num_workers=1,pin_memory=True,drop_last=True)
    
    dataset_val = dataset(folder_path,dataset_file,test_train_val="val",transform=transform,sen2_amount=1,  location=location)
    val_loader = DataLoader(dataset_val,batch_size=1, shuffle=True, num_workers=1,pin_memory=True,drop_last=True)
