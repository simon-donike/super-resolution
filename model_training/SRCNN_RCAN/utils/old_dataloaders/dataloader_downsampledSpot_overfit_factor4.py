...
class Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # down and upsample 
        im_spot6_down = Dataset.interpolate(im_spot6,size=125)
        im_spot6_down = Dataset.interpolate(im_spot6_down,size=500)
    
        # Perform Transform and change types
        im_spot6  = torch.from_numpy(im_spot6)
        im_spot6_down = torch.from_numpy(im_spot6_down)
        
        im_spot6 = im_spot6.float()
        im_spot6_down = im_spot6_down.float()
        
        # transform to 0..1 
        im_spot6  = torch.div(im_spot6,255.0)
        im_spot6_down = torch.div(im_spot6_down,255.0)
        
        # currently performing no normalization, only stretching to 0..1
        #im_spot6 = self.transform_spot(im_spot6)
        #im_spot6_down = self.transform_spot(im_spot6_down)
        
        return(im_spot6_down,im_spot6) #x,y

    
    
    def interpolate(img,size=25cv0):
        """
        Input:
            - Image
        Output:
