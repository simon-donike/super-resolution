...
#import utils.helper_functions_srcnn2
...
def loss_mae_lpips(a,b,weight_mae=0.1,weight_lpips=0.9):
    ...
    a,b = a.to(device),b.to(device) # send to cpu
...
def calculate_metrics(hr,lr,sr):
    import utils.helper_functions_srcnn2 as helper_functions
    ...
    interpolated = 0#interpolated[None] # add batch dimension to tensor
    lpips_int = 0#loss_lpips(hr,interpolated).item()
    psnr_int  = 0#loss_psnr(hr,interpolated).item()
    ssim_int  = 0#loss_ssim(hr,interpolated).item()
    mae_int   = 0#loss_mae(hr,interpolated).item()
