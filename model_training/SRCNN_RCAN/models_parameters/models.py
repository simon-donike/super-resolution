...
class SRCNN(nn.Module):
    ...
    def __init__(self, num_channels=3):
        ...
        self.conv1 = nn.Conv2d(num_channels, 64*3, kernel_size=9, padding=9 // 2)
        self.conv2 = nn.Conv2d(64*3, 32*3, kernel_size=5, padding=5 // 2)
        self.conv3 = nn.Conv2d(32*3, num_channels, kernel_size=5, padding=5 // 2)
...
class SRCNN_channel(nn.Module):
    # https://keras.io/examples/vision/super_resolution_sub_pixel/
    # https://mfarahmand.medium.com/cnn-based-single-image-super-resolution-6ffcd39ec993
    def __init__(self, num_channels=1):
        super(SRCNN_channel, self).__init__()
...
# set args here if none are passed
# TODO put actual values
# TODO put actual range

class RCAN():
    def __init__(self):
        # default settings
        self.args = {"n_resgroups":3,"n_resblocks":3,"n_feats":3,
                "reduction":3,"scale":4,"rgb_range":255,"n_colors":3,
                "res_scale":3,"data_train":"custom",
                "rgb_mean" :(0.4690, 0.4490, 0.4036), 
                "rgb_range":1} 
    def create_model(args):
        from models_parameters import rcan
        return(rcan.make_model(args))
        

        
class HighResNet():
    def get_model(config):
        from models_parameters.HighResNet import HRNet
        model = HRNet(config)
        return(model)