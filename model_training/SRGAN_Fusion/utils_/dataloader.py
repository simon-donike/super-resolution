...
class Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if sen2.shape!=(3,75,75):
            sen2 = np.random.uniform(low=0, high=10000, size=(3,75,75))
            sen2 = sen2.astype(int)
            print("FATAL ERROR IN DATALOADER: 1st loaded Sen2 image has invalid shape, initializing random instead! Following Images are likely still valid.")
        ...
        while sen2.size()!= torch.Size([3* self.sen2_amount,75,75]) or spot6.size()!=torch.Size([3, 300, 300]) or torch.sum(torch.where(sen2<=0,1,0)) > 2000*self.sen2_amount:
            #print("Warning in DataLoader: wrong tensor size or more than 2k*sen2_amount black pixels! Random image loaded instead")
            if self.transform=="spot6" or self.transform=="interpolate":
                break # dont do check if only doint spot 6 to spot 6
            sen2,spot6 = self.__getitem__(random.randint(0,self.__len__()-1))
